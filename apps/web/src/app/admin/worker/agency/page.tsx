...
import { Loader2, CheckCircle, Code2, Link as LinkIcon, Briefcase } from 'lucide-react';
import { useAuth } from '@/features/auth/AuthContext';
import toast from 'react-hot-toast';

export default function AgencyDeveloperDashboard() {
  ...
  const loadJobs = async () => {
    try {
      ...
      const response = await databases.listDocuments(dbId, 'agency_projects', [
      ...
      toast.error('Failed to load agency projects');
      console.error(err);
    } finally {
      setLoading(false);
    }
  ...
  const updateStatus = async (jobId: string, newStatus: string) => {
    try {
      ...
      await databases.updateDocument(dbId, 'agency_projects', jobId, payload);
    ...
    <div className="space-y-8 max-w-6xl mx-auto">
      <div>
        <h1 className="text-3xl font-extrabold text-white flex items-center gap-3">
          <Code2 className="h-8 w-8 text-fuchsia-500" />
          Developer Agency Deck
        </h1>
        <p className="text-slate-400 text-sm mt-1">Execute client software projects, manage development phases, and submit final deliverables.</p>
      </div>
        ...
        <div className="grid gap-6">
          {jobs.length === 0 ? (
            <div className="text-center py-12 border border-dashed border-slate-800 rounded-2xl bg-slate-950/50">
              <p className="text-slate-500 text-sm">No pending paid agency projects at the moment.</p>
            </div>
          ) : (
            jobs.map((job) => (
              <div key={job.$id} className="p-6 rounded-2xl border border-white/5 bg-slate-900/40 relative overflow-hidden">
                <div className="absolute top-0 right-0 p-4">
                  <span className={`px-3 py-1 rounded-full text-xs font-bold border ${
                    ...
                    job.status === 'in-progress' ? 'bg-fuchsia-500/10 text-fuchsia-400 border-fuchsia-500/20' : ''
                  }`}>
                    {job.status === 'paid' ? 'Ready for Dev' : 'Development In Progress'}
                  </span>
                </div>
                  ...
                  <div className="flex-1 space-y-4">
                    <div>
                      <span className="px-2 py-0.5 rounded text-[10px] uppercase font-bold tracking-wider bg-slate-800 text-slate-400 mb-2 inline-block">
                        {job.serviceType || 'Software Dev'}
                      </span>
                      <h3 className="text-xl font-bold text-white">{job.projectTitle || 'Untitled Project'}</h3>
                    ...
                    {job.status === 'in-progress' && (
                      <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 flex items-center gap-3">
                        ...
                        <input 
                          type="url" 
                          placeholder="Paste GitHub or Deployment URL for delivery..."
                  ...
                  <div className="flex flex-col gap-3 min-w-[200px] w-full md:w-auto">
                    {job.status === 'paid' && (
                      <button
                        ...
                        className="w-full py-3 px-4 rounded-xl font-bold text-sm bg-fuchsia-600 hover:bg-fuchsia-500 text-white transition-colors flex items-center justify-center gap-2"
                      >
                        {actioning === job.$id ? <Loader2 className="h-4 w-4 animate-spin" /> : <Code2 className="h-4 w-4" />}
                        Start Development
                      </button>
                    )}
                    ...
                    {job.status === 'in-progress' && (
                      <button
                        ...
                        title={!deliveryUrls[job.$id] ? "Please provide a delivery URL first" : "Submit Deliverable"}
                      >
