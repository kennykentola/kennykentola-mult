...
export default function AcademicLandingPage() {
  ...
  const handleWhatsAppSubmit = () => {
    if (!formData.title || !formData.universityName || !formData.department || !formData.description) {
      toast.error('Please fill in the required fields (University, Department, Title, Description)');
      return;
    }

    const message = `*New Academic Project Request*
...
Phone: ${formData.clientPhone || 'Not provided'}`;
    ...
    window.open(`https://wa.me/2348163571677?text=${encodedMessage}`, '_blank');
  };
            ...
            <form onSubmit={handleSubmit} className="text-left bg-[#050505] p-8 rounded-3xl border border-white/10 relative z-10 max-w-3xl mx-auto space-y-6 shadow-2xl">
              ...
              <div className="pt-4 flex flex-col sm:flex-row gap-4">
                <button 
                  type="submit" 
                  disabled={loading}
                  className="w-full flex-1 flex items-center justify-center gap-2 bg-gradient-to-r from-amber-500 to-orange-500 hover:opacity-90 text-white font-bold py-4 px-6 rounded-xl transition-all shadow-[0_0_30px_-10px_rgba(245,158,11,0.5)] disabled:opacity-50"
                >
                ...
                <button
                  ...
                  className="w-full flex-1 flex items-center justify-center gap-2 bg-[#25D366] hover:bg-[#20bd5a] text-white font-bold py-4 px-6 rounded-xl transition-all shadow-[0_0_30px_-10px_rgba(37,211,102,0.3)]"
                >
                ...
                <button 
                  type="button"
                  onClick={() => setIsConsultationModalOpen(true)}
                  className="w-full sm:w-auto flex items-center justify-center gap-2 bg-white/5 hover:bg-white/10 border border-white/10 text-white font-bold py-4 px-6 rounded-xl transition-all"
                >
